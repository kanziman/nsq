...
export function TutorChat({ className, context, speakers = [] }: TutorChatProps) {
  ...
  const filteredSpeakers = speakers.filter(s => s !== 'BOTH' && s !== 'NARRATOR');
  ...
  return (
    <div className={`flex flex-col h-full relative ${className || ''}`}>
      ...
      <div className="flex flex-wrap border-b border-hairline pb-4 mb-4 gap-2">
        {tabs.map(tab => {
          ...
          return (
            ...
            >
              {isGeneral ? <Bot className="w-3.5 h-3.5" /> : <User className="w-3.5 h-3.5" />}
              {name}
            </button>
          );
        })}
      ...
      {/* Chat History */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-6 scroll-smooth pr-2 pb-4">
                ...
                <div className="markdown-body">
                  {msg.content ? (
                    ...
                    components={{
                      ...
                      h2: ({node, ...props}) => <h2 className="text-base font-bold mt-3 mb-1 first:mt-0" {...props} />,
